    ...
    public function SavePage($tag, $body, $comment_on = "", $bypass_acls = false)
    {
        ...
        if ($rights) {
            ...
            if (! $oldPage = $this->LoadPage($tag)) {
				...
				$defaultWrite = $this->LoadAcl($tag, 'write', true)['list'] ;
				$defaultRead = $this->LoadAcl($tag, 'read', true)['list'];
				$defaultComment = $this->LoadAcl($tag, 'comment', true)['list'] ;
				
                // create default write acl. store empty write ACL for comments.
                $this->SaveAcl($tag, 'write', ($comment_on ? $user : $defaultWrite));

                // create default read acl
                $this->SaveAcl($tag, 'read', $defaultRead);

                // create default comment acl.
                $this->SaveAcl($tag, 'comment', ($comment_on ? '' : $defaultComment));
