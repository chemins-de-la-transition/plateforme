...
class BazarImportAction extends YesWikiAction
{
    ...
    public function run()
    {
        ...
        $output .= '<form method="post" action="'.$this->wiki->href('', $this->wiki->getPageTag(), $urlParams).'" '.
            'enctype="multipart/form-data" class="form-horizontal">'."\n";

    
        // le fichier cvs vient d'être téléchargé, on le traite
        if (isset($_POST['submit_file'])) {
            $output .= $this->importFile();
        } elseif (isset($_POST['importfiche'])) {
            $output .= $this->importEntry();
        } else {
    ...
    protected function importFile():?string
    {
        // retrocompatibilty
        $id = $this->arguments['id'];
        
        $row = 1;
        $val_formulaire = $this->formManager->getOne($id);
                        ...
                        if ($row == 1) {
                            ...
                            if (!empty($valeur['bf_titre'])) {
                                ...
                                if ($this->wiki->UserIsAdmin()) {
                                    ...
                                    $valeur['statut_fiche'] = $this->wiki->config['BAZ_ETAT_VALIDATION'];
                                }
    ...
    protected function importEntry():?string
    {
        // retrocompatibilty
        $id = $this->arguments['id'];
    ...
    protected function renderImportForm(): ?string
    {
        ...
        $resultat = $this->formManager->getAll();
